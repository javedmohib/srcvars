...
#' @export
#'@examples
#' find_files(folder_path = "K:/QILT/GOS/2023", subfol der_path = c("sample", "feb"), file_extension = "xlsx", file_name = c("Spec", "custom"), match_all = TRUE, strict = TRUE)
find_files <- function(folder_path, subfolder_path = "", file_name, file_extension = "", match_all = TRUE, strict = FALSE){
  ...
  for (file in all_files){
    if (!grepl("^\\.", file) && # remove the files that start with "."
